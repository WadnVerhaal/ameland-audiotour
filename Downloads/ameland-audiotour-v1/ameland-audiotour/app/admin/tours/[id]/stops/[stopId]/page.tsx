...
export default async function AdminStopDetailPage({
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-4 px-4 py-8">
      ...
      <form action={updateStopAction.bind(null, id, stopId)} className="space-y-4 rounded-3xl bg-white p-4 shadow-soft">
        <div className="grid gap-4 md:grid-cols-2">
          <label className="block space-y-2">
            <span className="text-sm font-medium">Titel stop (Nederlands)</span>
            <input name="title" defaultValue={stop.title} className="w-full rounded-2xl border px-4 py-3" required />
          </label>

          <label className="block space-y-2">
            <span className="text-sm font-medium">Titel stop (Engels)</span>
            <input name="title_en" defaultValue={stop.title_en ?? ''} className="w-full rounded-2xl border px-4 py-3" />
          </label>
        </div>

        <label className="block space-y-2">
          <span className="text-sm font-medium">Titel stop (Duits)</span>
          <input name="title_de" defaultValue={stop.title_de ?? ''} className="w-full rounded-2xl border px-4 py-3" />
        </label>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="block space-y-2">
            ...
            <input name="lat" defaultValue={String(stop.lat ?? '')} className="w-full rounded-2xl border px-4 py-3" required />
          </label>
          <label className="block space-y-2">
            <span className="text-sm font-medium">Longitude</span>
            <input name="lng" defaultValue={String(stop.lng ?? '')} className="w-full rounded-2xl border px-4 py-3" required />
          </label>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="block space-y-2">
            ...
            <input name="audio_url" defaultValue={stop.audio_url ?? ''} className="w-full rounded-2xl border px-4 py-3" />
          </label>
          <label className="block space-y-2">
            <span className="text-sm font-medium">Afbeelding URL</span>
            <input name="image_url" defaultValue={stop.image_url ?? ''} className="w-full rounded-2xl border px-4 py-3" />
          </label>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="block space-y-2">
            ...
            <input type="number" name="trigger_radius_meters" defaultValue={stop.trigger_radius_meters} className="w-full rounded-2xl border px-4 py-3" />
          </label>
          <label className="block space-y-2">
            <span className="text-sm font-medium">Duur audio (sec)</span>
            <input type="number" name="estimated_duration_seconds" defaultValue={stop.estimated_duration_seconds} className="w-full rounded-2xl border px-4 py-3" />
          </label>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button formAction={deleteStopAction.bind(null, id, stopId)} className="rounded-2xl bg-rose-600 px-4 py-3 text-white">
            Verwijderen
          </button>
        </div>
      </form>
