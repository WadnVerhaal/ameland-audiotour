...
const COPY = {
  nl: {
    ...
    directRoute: 'Hemelsbrede lijn',
    routeLoading: 'Route bijwerken…',
    routeUnavailable: 'Wandelroute tijdelijk niet beschikbaar',
  ...
  en: {
    ...
    directRoute: 'Direct line',
    routeLoading: 'Updating route…',
    routeUnavailable: 'Walking route temporarily unavailable',
  ...
  de: {
    walkingRoute: 'Fußweg',
    directRoute: 'Direkte Linie',
    routeLoading: 'Route wird aktualisiert…',
    routeUnavailable: 'Fußweg vorübergehend nicht verfügbar',
...
function NavigationControls({
  ...
  function showRoute() {
    setCameraMode('route')
    fitRoute(map, route, programmaticMoveRef)
  }
...
export function PlayerMap({
  ...
  const [routeMeta, setRouteMeta] = useState<RouteMeta>({ distanceMeters: null, durationSeconds: null })
  ...
  useEffect(() => {
    ...
    if (target) setRoute(location ? [[location.lat, location.lng], target] : [target])
    else setRoute([])
    setRouteStatus(target ? 'loading' : 'idle')
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        setRouteMeta({
          distanceMeters: fallbackRoute.length > 1 ? distanceMeters(fallbackRoute[0], fallbackRoute[1]) : null,
  ...
  const routeDistance = formatDistance(routeMeta.distanceMeters, language)
  const routeDuration = formatDuration(routeMeta.durationSeconds, language)
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-slate-950">
        ...
        {routeLine.length > 1 ? (
          <>
            ...
            <Polyline
              positions={routeLine}
              pathOptions={{
                color: routeStatus === 'fallback' ? '#f59e0b' : '#0f766e',
                weight: 6,
                opacity: 1,
                lineCap: 'round',
                lineJoin: 'round',
                dashArray: routeStatus === 'fallback' ? '10 10' : undefined,
              }}
            />
          </>
        ) : null}
      ...
      <div className="pointer-events-none absolute left-3 top-3 z-[750] max-w-[68%] rounded-2xl border border-white/15 bg-slate-950/88 px-3 py-2 text-white shadow-2xl backdrop-blur">
        ...
        {routeStatus === 'fallback' ? (
          <p className="mt-1 text-[11px] font-semibold text-amber-200">{t.routeUnavailable}</p>
        ) : null}
      </div>
    </div>
  )
